...
//     vec2 centerPos = pixelPos - itemSize * 0.5;
//
//     float r = min(
//         ubuf.cornerRadius,
//         min(itemSize.x, itemSize.y) * 0.5
//     );
//
...
//
//     if (ubuf.fillMode == 1 || ubuf.fillMode == 2) {
//         float itemAspect = itemSize.x / itemSize.y;
//         float srcAspect  = sourceSize.x / sourceSize.y;
//
//         if (ubuf.fillMode == 1) { // PreserveAspectFit
//             if (srcAspect > itemAspect)
//                 uv.y = (uv.y - 0.5) * (srcAspect / itemAspect) + 0.5;
//             else
//                 uv.x = (uv.x - 0.5) * (itemAspect / srcAspect) + 0.5;
//
//             mask *= step(0.0, uv.x) * step(uv.x, 1.0)
...
//
//     float opacity = mask * ubuf.imageOpacity * ubuf.qt_Opacity;
//
//     // Correct premultiplied alpha output
//     fragColor = vec4(tex.rgb * opacity, tex.a * opacity);
// }
